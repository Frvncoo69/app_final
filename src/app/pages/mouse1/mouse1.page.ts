...
export class Mouse1Page implements OnInit {
  ...
  idProducto: number = 5; // ID del mouse (ajustar según tu base de datos)
  ...
  constructor(
    ...
    this.activedroute.queryParams.subscribe((res) => {
      if (this.router.getCurrentNavigation()?.extras.state) {
        this.productoSolo = this.router.getCurrentNavigation()?.extras?.state?.['mouseVa'];
      }
    });
  }
  ...
  async agregarAlCarrito() {
    ...
    try {
      ...
      this.alertasService.presentAlert('Añadido al Carrito', 'El mouse fue añadido correctamente.');
    } catch (error) {
